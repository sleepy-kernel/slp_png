...
int main(void) {
    ...
    start = clock();
    if (!slp_image_format(&a)){printf("format fail\n");return 1;}
    end = clock();

    printf("format time: %.6fs\n", (double)(end - start) / CLOCKS_PER_SEC);
    



    start = clock();
    if (!slp_image_convert_to_16bit(&a)){printf("cvt16 fail\n");return 1;}
    end = clock();

    printf("cvt16 time: %.6fs\n", (double)(end - start) / CLOCKS_PER_SEC);




    start = clock();
    if (!slp_image_convert_to_8bit(&a)){printf("cvt8 fail\n");return 1;}
    end = clock();

    printf("cvt8 time: %.6fs\n", (double)(end - start) / CLOCKS_PER_SEC);




    start = clock();
    if (!slp_image_format(&a))
    {printf("unformat fail\n");return 1;}
    end = clock();

    printf("unformat time: %.6fs\n", (double)(end - start) / CLOCKS_PER_SEC);
